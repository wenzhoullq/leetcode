## 模板
本质是双指针甚至三指针，固定一个left;   right指针不断移动，寻找到**满足的**不断对left进行滑动至**极限**,**数字类滑动窗口**有一个很大的前提，就是**保证所有的数字都是非负的**，关键词是连续，最多包含一个（删除一个）
```java
...
class Solution {
    public int longestOnes(int[] nums, int k) {
        int left=0,right=0,use=0,ans=0,length=nums.length;
        while(right<length){
            if(nums[right]==0){
                while(use==k){
                    if(nums[left]==0) {
...
 
## 思路
从题目的描述中得知是滑动窗口，但是需要最大值和最小值，有两种方法，方法一是维护两个单调队列，分别记录最大值和最小值，另一种是用TreeMap同时维护最大值和最小值
```java
class Solution {
    public int longestSubarray(int[] nums, int limit) {
        ...
        for(int i=0;i<nums.length;i++){
            ...
            while(Math.abs(m.firstKey()-m.lastKey())>limit){
